...
function draw() { 
    ...
    // in the image
    let obj = findColor(video, colorToMatch, tolerance);
    // if we got a result (is not undefined)
    // then draw a circle in that location
    if (obj !== undefined) {
        let firstPx = obj.vector;
        playSynth(obj.x, obj.y);
        fill(colorToMatch);
        stroke(255);
        strokeWeight(2);
...
function playSynth(x, y) {
    userStartAudio();
    let multiplier = (windowWidth + windowHeight) / 500;
    console.log(multiplier)
    polySynth.play((x + y) * multiplier, .1, 0, .1);
}
  ...
  for (let y=0; y<input.height; y++) {
    for (let x=0; x<input.width; x++) {
      ...
      if (r >= matchR-tolerance && r <= matchR+tolerance &&
          ...
          obj = new temp(createVector(x, y), x, y);
          return obj;
      }
    }
  }
